...
public class UserDAOImpl implements IUserDAO {
    ...
    public void save(User user) throws DAOException {
        ...
        try (Connection connection = connectionPool.takeConnection();
            ...
            try (ResultSet rs = preparedStatement.getGeneratedKeys()) {
                ...
                saveFavoriteGenres(user.getId(), user.getFavoriteGenres());
            }
    ...
    public void saveFavoriteGenres(int userId, List<Genre> genreList) throws DAOException {
        ...
        try (Connection connection = connectionPool.takeConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(INSERT_FAVORITE_GENRE);
             PreparedStatement preparedStatement1 = connection.prepareStatement(DELETE_FAVORITE_GENRE)
        ) {
        ...
        try (Connection connection = connectionPool.takeConnection();
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                if (rs.next()) {
                    ...
                    user.setFavoriteGenres(getFavoriteGenresOfUser(id));
                } else {
        ...
        try (Connection connection = connectionPool.takeConnection();
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                if (rs.next()) {
                    ...
                    user.setDateRegistration(rs.getTimestamp(7).toLocalDateTime());
                    user.setRole(Role.valueOf(rs.getString(8).toUpperCase()));
                    user.setFavoriteGenres(getFavoriteGenresOfUser(user.getId()));
                } else {
                    throw new DAOException("Error getting user id");
                }
            }
    ...
    public List<Genre> getFavoriteGenresOfUser(int userId) throws DAOException {
        ...
        try (Connection connection = connectionPool.takeConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_FAVORITE_GENRES)) {
            preparedStatement.setInt(1, userId);
            try (ResultSet rs = preparedStatement.executeQuery()) {
                Genre genre = null;
                while (rs.next()) {
